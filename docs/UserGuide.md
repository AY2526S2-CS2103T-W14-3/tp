...
Install AddressMe on the new device, run it once to generate the data folder, then replace the empty addressbook.json
with your existing data file from the old device.
...
**A:** Yes. There is no deduplication. You may want to use tags or visit dates to distinguish between records instead of
creating duplicates.
